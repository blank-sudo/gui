...
1. `monitor_target = Vout_target / ratio`（ratio 定义为 output:monitor）
...
本工具当前固定使用 `CR`（`\\r`）作为命令结束符，并会在日志打印发送 HEX，便于核对。  
例如 `enable=1` 会打印类似：`HEX=65 6e 61 62 6c 65 3d 31 0d`（最后 `0d` 就是 CR）。
