...
class SerialVoltageApp:
    ...
    def __init__(self, root: tk.Tk) -> None:
        ...
        self.monitor_ratio_var = tk.DoubleVar(value=20.0)  # output:monitor = 20:1
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Label(calib_frame, text="output:monitor 比例").grid(row=0, column=4, **pad)
    ...
    def send_command(self, cmd: str, expect_reply: bool = False) -> None:
        ...
        try:
            ...
            # 按设备协议固定使用 CR 结束符
            wire = (clean_cmd + "\r").encode("ascii")
            self.ser.write(wire)
            self.log(f"-> {clean_cmd} [EOL=CR, HEX={wire.hex(' ')}]")
            if expect_reply:
                self.ser.flush()
        except Exception as e:
            self.log(f"发送失败: {e}")
    ...
    def voltage_to_value(self, v_out: float) -> tuple[int, float]:
        ...
        if abs(ratio) < 1e-12:
            raise ValueError("output:monitor 比例不能为 0")

        # ratio 定义为 output:monitor，例如 20 表示 Vout = 20 * Vmonitor
